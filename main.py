...
def dashboard():
    try:
        eggs_today = sum(x.quantity for x in Egg.query.all())
        total_eggs = sum(x.quantity for x in Egg.query.all())
        total_feed = sum(x.quantity for x in Feed.query.all())
        revenue = sum(x.total for x in Sale.query.all())
    ...
    return page(
        ...
        <section class="cards">
            ...
            <article class="card"><span>Feed Used</span><strong>{{ "%.1f"|format(total_feed) }} kg</strong></article>
        </section>
